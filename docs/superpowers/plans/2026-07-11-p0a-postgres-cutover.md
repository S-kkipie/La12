...
- [ ] **Step 2: Typecheck — inferred types unchanged, consumers still compile**

Run: `cd web && pnpm exec tsc --noEmit`
Expected: PASS. No new errors in `lib/*.ts` or `app/api/**` (they import `Club`/`Round`/etc.; `serial` still infers `id: number` on select and optional on insert, `boolean` infers `boolean`, `timestamp` infers `Date`, `text` infers `string` — identical to the sqlite shapes).

- [ ] **Step 3: Cross-check the Better Auth tables against the CLI (drift guard)**

The spec requires the `user`/`session`/`account`/`verification` shapes match Better Auth's Postgres adapter exactly. The hand-port above is verified against the generator:
...
Then diff the four auth tables' columns/types against `db/schema.ts`. Expected: same column names, same types (`text` id, `boolean` email_verified, `timestamp` \*_at, `text` role). Reconcile any difference into `db/schema.ts` (the generator wins on auth tables). Discard `/tmp/ba-pg-schema.ts` — it is a reference only; do not overwrite `db/schema.ts` (it also holds the app tables).

- [ ] **Step 4: Commit**

```bash
cd /home/skkippie/work/AI-DO/La12
git add web/db/schema.ts
git commit -m "feat(p0a): port Drizzle schema sqlite-core -> pg-core"
```

---

## Task 3: Swap the Drizzle client to node-postgres + push schema

**Files:**
- Rewrite: `web/lib/db.ts`
- Rewrite: `web/drizzle.config.ts`
- Create: `web/lib/db.roundtrip.test.ts`

**Interfaces:**
- Consumes: `DATABASE_URL` (Task 1); pg schema (Task 2).
- Produces: `db` — a `drizzle-orm/node-postgres` instance with `{ schema, casing: "snake_case" }`, importable exactly as before (`import { db } from "@/lib/db"`). Tables physically created in Postgres.

- [ ] **Step 1: Rewrite `web/lib/db.ts`**
...
- [ ] **Step 2: Rewrite `web/drizzle.config.ts`**
...
- [ ] **Step 4: Write the failing round-trip test `web/lib/db.roundtrip.test.ts`**
...
```bash
cd /home/skkippie/work/AI-DO/La12
git add web/lib/db.ts web/drizzle.config.ts web/lib/db.roundtrip.test.ts web/drizzle
git commit -m "feat(p0a): drizzle node-postgres client + schema push + round-trip test"
```
...
**Interfaces:**
- Consumes: `db` (Task 3); `better-sqlite3` (already installed).
